...
const NAMES = [
  "Ali Hassan", "Sara Khan", "John Doe", "Maria Garcia", "Ahmed Raza",
...
async function getAnalytics() {
  ...
  const totalTreated = await prisma.patient.count({
  ...
  return {
    totalAdmitted,
    totalTreated,
    treatmentLog: treatmentLog.map((t: any) => ({
      time: t.treatedAt?.getTime() ?? 0,
      severity: t.priority,
      waitSeconds: t.waitSeconds,
      name: t.name,
    })),
    boostEvents: boostEvents.map((b: any) => ({
      time: b.time.getTime(),
...
export async function POST(req: Request) {
  ...
  const queueCount = await prisma.patient.count({
    where: { status: "waiting" },
  });

  if (data.simulate) {
    if (queueCount >= 10)
      return Response.json({ error: "Queue full" }, { status: 429 });
    ...
    return Response.json({
      message: "Simulated patient added",
      patient: serializePatient(patient),
      queue: queue.map(serializePatient),
    });
  }

  if (!data.name || !data.condition || !data.priority)
    return Response.json(
      { error: "name, condition, and priority are required" },
      { status: 400 }
    );
  ...
  if (queueCount >= 10)
    return Response.json(
      { error: "Queue full. Max 10 patients." },
      { status: 429 }
    );
  ...
  return Response.json({
    ...
    entry: serializePatient(patient),
    queue: queue.map(serializePatient),
  });
}
...
export async function PATCH(req: Request) {
  ...
  if (body.override) {
    ...
    await prisma.patient.update({
      ...
      data: {
        ...
        boosted: true,
        boostReason: reason || "Doctor override",
      },
    });
  ...
  if (body.treatById) {
    ...
    if (queue.length === 0)
      return Response.json({ error: "Queue is empty" }, { status: 404 });

    patient = queue[0];
  }

  const waitSeconds = Math.floor(
    (Date.now() - patient.enqueuedAt.getTime()) / 1000
  );
  ...
  return Response.json({
    ...
    treatedPatient: serializePatient(treatedPatient),
  });
}
